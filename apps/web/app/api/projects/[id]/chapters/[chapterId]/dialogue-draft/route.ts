...
import { mergeSpeechAndVoiceProfile, type MergedSpeechProfile } from "@manga-ai-studio/core";
...
export async function POST(req: Request, ctx: Ctx) {
  ...
  const characters = await prisma.character.findMany({
    where: { projectId },
    select: {
      id: true, name: true, roleType: true,
      speechProfile: true, stableSpeechDNA: true,
      voiceRegister: true, voiceSentenceLength: true, voiceVocabularyStyle: true,
      voiceEmotionalLeak: true, voiceSarcasmLevel: true, voiceAggressionLevel: true,
      voiceSilenceFrequency: true, voiceFavoriteExpressions: true, voiceForbiddenExpressions: true,
      voiceForbiddenPatterns: true, voiceSpeechRules: true, voiceThreatenStyle: true,
      voiceLieStyle: true, voiceSeductionStyle: true, voiceInnerMonologueStyle: true,
      voiceExamplesCanonical: true,
    },
    orderBy: { createdAt: "asc" },
  });
  const characterNameById: Record<string, string> = {};
  const knownCharacterIds: string[] = [];
  const speechProfilesByCharacterId = new Map<string, MergedSpeechProfile>();
  for (const c of characters) {
    characterNameById[c.id] = c.name;
    knownCharacterIds.push(c.id);
    const merged = mergeSpeechAndVoiceProfile({
      speechProfile: c.speechProfile as Record<string, unknown> | null,
      stableSpeechDNA: c.stableSpeechDNA as Record<string, unknown> | null,
      voiceRegister: c.voiceRegister,
      voiceSentenceLength: c.voiceSentenceLength,
      voiceVocabularyStyle: c.voiceVocabularyStyle,
      voiceEmotionalLeak: c.voiceEmotionalLeak,
      voiceSarcasmLevel: c.voiceSarcasmLevel,
      voiceAggressionLevel: c.voiceAggressionLevel,
      voiceSilenceFrequency: c.voiceSilenceFrequency,
      voiceFavoriteExpressions: asStringArray(c.voiceFavoriteExpressions),
      voiceForbiddenExpressions: asStringArray(c.voiceForbiddenExpressions),
      voiceForbiddenPatterns: asStringArray(c.voiceForbiddenPatterns),
      voiceSpeechRules: asStringArray(c.voiceSpeechRules),
      voiceThreatenStyle: c.voiceThreatenStyle,
      voiceLieStyle: c.voiceLieStyle,
      voiceSeductionStyle: c.voiceSeductionStyle,
      voiceInnerMonologueStyle: c.voiceInnerMonologueStyle,
      voiceExamplesCanonical: Array.isArray(c.voiceExamplesCanonical)
        ? (c.voiceExamplesCanonical as Array<Record<string, unknown>>)
        : null,
    });
    speechProfilesByCharacterId.set(c.id, merged);
  }
