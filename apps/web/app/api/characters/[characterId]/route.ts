...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const character = await prisma.$transaction(async (tx) => {
    ...
    // Shim transitoire : miroir colonnes plates ← identité résolue.
    // Supprimable quand plus aucun lecteur ne tape les colonnes plates directement.
    const rRaw = refreshed as unknown as Record<string, unknown>;
    const identitySource: CharacterIdentitySource = {
      name: refreshed.name,
      gender: typeof rRaw.gender === "string" ? rRaw.gender : null,
      appearance: typeof rRaw.appearance === "string" ? rRaw.appearance : null,
      hairColor: typeof rRaw.hairColor === "string" ? rRaw.hairColor : null,
      eyeColor: typeof rRaw.eyeColor === "string" ? rRaw.eyeColor : null,
      outfitDefault: typeof rRaw.outfitDefault === "string" ? rRaw.outfitDefault : null,
      roleType: typeof rRaw.roleType === "string" ? rRaw.roleType : null,
      visualProfile: rRaw.visualProfile && typeof rRaw.visualProfile === "object" ? (rRaw.visualProfile as Record<string, unknown>) : null,
      bodyState: rRaw.bodyState && typeof rRaw.bodyState === "object" ? (rRaw.bodyState as Record<string, unknown>) : null,
      wardrobeProfile: rRaw.wardrobeProfile && typeof rRaw.wardrobeProfile === "object" ? (rRaw.wardrobeProfile as Record<string, unknown>) : null,
      continuityProfile: rRaw.continuityProfile && typeof rRaw.continuityProfile === "object" ? (rRaw.continuityProfile as Record<string, unknown>) : null,
      stableVisualDNA: rRaw.stableVisualDNA && typeof rRaw.stableVisualDNA === "object" ? (rRaw.stableVisualDNA as Record<string, unknown>) : null,
    };
    const resolvedId = resolveCharacterIdentity(identitySource);

    const mirrorData: Record<string, string | null> = {};
    if (resolvedId.hairColor && resolvedId.hairColor !== (rRaw.hairColor ?? null)) mirrorData.hairColor = resolvedId.hairColor;
    if (resolvedId.eyeColor && resolvedId.eyeColor !== (rRaw.eyeColor ?? null)) mirrorData.eyeColor = resolvedId.eyeColor;
    if (resolvedId.appearanceText && resolvedId.appearanceText !== (rRaw.appearance ?? null)) mirrorData.appearance = resolvedId.appearanceText;
    if (resolvedId.outfit && resolvedId.outfit !== (rRaw.outfitDefault ?? null)) mirrorData.outfitDefault = resolvedId.outfit;
    if (resolvedId.gender && resolvedId.gender !== (rRaw.gender ?? null)) mirrorData.gender = resolvedId.gender;
    ...
    if (Object.keys(mirrorData).length > 0) {
      ...
      Object.assign(refreshed, mirrorData);
    }
    ...
    const score = computeCanonPackScore({
      ...
      resolvedHairColor: resolvedId.hairColor,
      resolvedEyeColor: resolvedId.eyeColor,
      resolvedAppearance: resolvedId.appearanceText,
      resolvedOutfit: resolvedId.outfit,
      resolvedGender: resolvedId.gender,
    });
