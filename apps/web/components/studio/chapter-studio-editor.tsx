...
import { rebuildVisualWorldContract } from "@/lib/chapter-studio/rebuild-visual-world-contract";
import {
...
export function ChapterStudioEditor({ projectId, chapterId }: { projectId: string; chapterId: string }) {
  ...
  const [activeFlowStep, setActiveFlowStep] = useState<ChapterFlowStepId>("characters");
  ...
  const loadStudio = useCallback(async () => {
    ...
    setActiveFlowStep(normalizeFlowStepId(mapStudioStepToFlowStep(json.snapshot.currentStep)));
    setLoading(false);
  }, [chapterId, projectId]);
  ...
  const updateDraft = useCallback((next: ChapterStudioData, step = activeStudioStep) => {
    let patched = next;
    try {
      const vw = rebuildVisualWorldContract({ chapterId, draft: next });
      patched = { ...next, visualWorldContract: vw };
    } catch {
      // keep next as-is if rebuild fails (incomplete data is tolerated)
    }
    setDraft(patched);
    setActiveStudioStep(step);
    if (autosaveRef.current) window.clearTimeout(autosaveRef.current);
    autosaveRef.current = window.setTimeout(() => {
      void save(patched, step);
    }, 900);
  }, [activeStudioStep, chapterId, save]);
  ...
  const generateOutlines = useCallback(async () => {
    if (!draft?.intent?.shortPitch) {
      ...
      goToFlowStep("story", "studio-short-pitch", "intent");
      return;
    }
    ...
    <div className="grid gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
      ...
      <div className="space-y-6">
        ...
        <ChapterStudioFlowBody
          ...
          storyWarnings={storyWarnings}
          charactersIssues={charactersIssues}
          charactersWarnings={charactersWarnings}
          locationsIssues={locationsIssues}
          locationsWarnings={locationsWarnings}
          livingWorldIssues={livingWorldIssues}
          livingWorldWarnings={livingWorldWarnings}
          ...
          onContinueFromStep={(fromStep) => {
            const next = nextFlowStep(fromStep);
            if (!next || !draft) return;
            if (!expertMode && fromStep === "story") {
              void runAutofill("all_missing").then(() => {
                goToFlowStep("plan", null, "production_plan");
              });
              return;
            }
