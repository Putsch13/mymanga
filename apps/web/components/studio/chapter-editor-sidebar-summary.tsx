...
const EXPRESS_OPTIONAL_STEPS = new Set<ChapterFlowStepId>(["locations", "living_world"]);
...
export function ChapterEditorSidebarSummary({
  ...
  /** Masque décors / PNJ dans la nav quand le parcours express est actif. */
  expressMode?: boolean;
  ...
  return (
    <aside className="space-y-4 xl:sticky xl:top-6 xl:self-start" data-testid="studio-sidebar">
      ...
      <div className="space-y-3">
        {flowSteps
          .filter((step) => !expressMode || !EXPRESS_OPTIONAL_STEPS.has(step.id))
          .map((step) => (
