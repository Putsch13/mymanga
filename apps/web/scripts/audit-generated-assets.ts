...
async function auditChapter(chapterId: string): Promise<AuditResult> {
  ...
  const panelNumbers = new Map<number, string[]>();
  ...
  for (const img of allImages) {
    ...
    if (!img.storageKey && img.status === "completed") {
      ...
      issues.push({
        ...
        sceneImageId: img.id,
        panelNumber,
        message: "Completed image missing storageKey",
      });
    }
    ...
    if (panelNumber != null) {
      const existing = panelNumbers.get(panelNumber) ?? [];
      existing.push(img.id);
      panelNumbers.set(panelNumber, existing);
    }
  }

  for (const [panelNum, ids] of panelNumbers) {
    if (ids.length > 1) {
      ...
      issues.push({
        ...
        message: `${ids.length} images for same panel number`,
        data: { imageIds: ids },
      });
    }
  }
...
function formatReport(result: AuditResult): string {
  const lines: string[] = [
    ...
    `| Missing storageKey | ${result.summary.missingStorageKey} |`,
