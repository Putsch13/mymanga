...
export function buildPremiumReadinessDashboard(input: {
  ...
  if (vwRaw != null) {
    ...
    if (!parsed.success) {
      pushCatalog("VISUAL_WORLD_CONTRACT_FAILED", parsed.error.message, "decor");
    } else {
    ...
    pushCatalog("VISUAL_WORLD_CONTRACT_FAILED", "Aucun décor ni entité défini — ajoute au moins un lieu dans l'étape Décors.", "decor");
  }
