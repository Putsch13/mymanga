...
function mapNpcGroups(entities: ChapterEntities) {
  return entities.npcs.map((n) => {
    const appearance = (n.appearance ?? "").trim();
    const outfit = typeof (n as Record<string, unknown>).outfit === "string"
      ? ((n as Record<string, unknown>).outfit as string).trim()
      : "";
    const behavior = (n.behavior ?? "").trim();

    return {
      id: n.id,
      label: n.label,
      role: (n.narrativeRole ?? "").trim() || "support",
      visualProfile: appearance || n.label,
      outfit: outfit || null,
      silhouette: behavior || "silhouette lisible manga",
