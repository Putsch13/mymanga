/**
 * P1.1 — Source de vérité unique pour le canon visuel d'un personnage.
 *
 * Jusqu'ici, 4 sources (CharacterVisualLock, CharacterFingerprint,
 * stableVisualDNA, visualProfile/bodyState/wardrobeProfile) étaient lues de
 * façon ad-hoc par `generate-visual/route.ts`, `retry/route.ts`,
 * `chapter-studio.ts` et `narrative-pass.ts` → dérive et incohérences.
 *
 * Ce module centralise la résolution avec un ordre de priorité strict :
 *   1. CharacterVisualLock actif (canonical source of truth)
 *   2. CharacterFingerprint (colonne dédiée, stable)
 *   3. stableVisualDNA (JSON legacy)
 *   4. visualProfile + bodyState + wardrobeProfile (champs de base)
 *
 * Lecture seule : ne déclenche AUCUN write. L'input est un Character enrichi
 * (select/include Prisma à la charge de l'appelant).
 */
...
/**
 * Extrait toutes les URLs canoniques "stables" connues pour un personnage.
 * On refuse silencieusement les URLs temporaires/signées (isStableImageUrl).
 */
