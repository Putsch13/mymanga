...
export interface CanonPackScoreInput {
  ...
  /**
   * Identité résolue (via resolveCharacterIdentity).
   * Quand fournie, les champs résolus complètent les colonnes plates
   * pour le calcul de complétude — un perso rempli via le configurateur
   * obtient un score réaliste même si les colonnes plates sont vides.
   */
  resolvedHairColor?: string | null;
  resolvedEyeColor?: string | null;
  resolvedAppearance?: string | null;
  resolvedOutfit?: string | null;
...
export function computeCanonPackScore(input: CanonPackScoreInput): CanonPackScoreResult {
  ...
  const identityChecks = [
    ...
    { key: "gender", ok: isNonEmptyString(input.gender) || isNonEmptyString(input.resolvedGender) },
  ...
  // Colonnes plates OU identité résolue (configurateur / stableVisualDNA)
  const appearanceChecks = [
    { key: "appearance", ok: isNonEmptyString(input.appearance) || isNonEmptyString(input.resolvedAppearance) },
    { key: "hairColor", ok: isNonEmptyString(input.hairColor) || isNonEmptyString(input.resolvedHairColor) },
    { key: "eyeColor", ok: isNonEmptyString(input.eyeColor) || isNonEmptyString(input.resolvedEyeColor) },
    { key: "outfitDefault", ok: isNonEmptyString(input.outfitDefault) || isNonEmptyString(input.resolvedOutfit) },
  ];
