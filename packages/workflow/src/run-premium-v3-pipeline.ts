  ...
  try {
    ...
    if (projectFormat === "manga") {
      ...
      if (workingBlueprints && workingBlueprints.length > 0) {
        ...
        if (typesLine.length > 0) {
          logPipelineInfo("pipeline.v3.entity.coverage.types", { message: `${typesLine}` });
        }
        ...
        if (narrativeVar.panelsAdjusted > 0) {
          logPipelineInfo("pipeline.v3.narrative.variation", { panelsAdjusted: narrativeVar.panelsAdjusted });
        }
        ...
        if (!mangaQa.ok) {
          logPipelineError("pipeline.v3.manga.structure.qa.failed", { reason: mangaQa.summary });
          if (input.premiumV3OnlyEnabled) {
    ...
    if (approvedPlanDriven) {
      ...
      const resolvedNarrativeContract: IntentNarrativeContract | null = (() => {
        ...
        // Même si l'intent est vide, on construit un contrat minimal avec les
        // personnages/lieux connus pour que le Story Architect ne travaille
        // jamais sans contraintes quand un ChapterIntentContract existe.
      ...
      const intentContractRecord = input.chapterIntentContract as
        | { era?: unknown; setting?: unknown }
        | null;
      const storyPassResult = await runStoryPass({
        ...
        era: typeof intentContractRecord?.era === "string" ? intentContractRecord.era : null,
        setting: typeof intentContractRecord?.setting === "string" ? intentContractRecord.setting : null,
    ...
    logPipelineInfo("pipeline.v3.beat.coverage.qa", { message: `${formatBeatCoverageQaLog(beatCoverageQa)}` });
    if (!beatCoverageQa.ok && input.premiumV3OnlyEnabled) {
      logPipelineError("pipeline.v3.beat.coverage.qa.blocked__uncovered_beats_detected", {});
      throw new Error(
    ...
    if (emotionalArcSteps.length > 0) {
      ...
      logPipelineInfo("pipeline.v3.emotional.arc.qa", { message: `${formatEmotionalArcQaLog(emotionalArcQa)}` });
    ...
    logPipelineInfo("pipeline.v3.interaction.qa", { message: `${formatInteractionQaLog(interactionQa)}` });
    ...
    try {
      ...
      logPipelineWarn("pipeline.v3.chapter.visual.contract.persist_failed", { chapterId: input.chapterId, message: `persist_failed chapterId=${input.chapterId} ${msg}` });
    }
    ...
    logPipelineInfo("pipeline.v3.visual.contract.contract_confirmed_d_blueprint_confirmed_d_optional_d_suspic", {
      value: {
        contractConfirmedLen: contractSanitized.requiredConfirmed.length,
        blueprintConfirmedLen: blueprintSanitized.requiredConfirmed.length,
        optionalLen: blueprintSanitized.optional.length,
        suspiciousLen: blueprintSanitized.suspicious.length,
        rejectedLen: blueprintSanitized.rejected.length,
        parasitePolicy,
        contractParasiteNote: String(contractHasRequiredSlices && parasitePolicy !== "keep_all"),
      },
    });
    ...
    if (remaining.rejectedGaps.length > 0) {
      logPipelineWarn("pipeline.v3.visual.coverage.rejected_gaps_ignored", {
        value: {
    ...
    if (remaining.repairableGaps.length > 0 || remaining.softGaps.length > 0) {
      logPipelineWarn("pipeline.v3.visual.coverage.soft_gaps_after_repair", {
        value: {
    ...
    if (visualCoverageStatus !== "ok") {
      logPipelineInfo("pipeline.v3.visual.coverage", { status: visualCoverageStatus });
    }
    ...
    try {
      ...
      logPipelineInfo("pipeline.v3.props.qa", { message: `${formatPropsQaLog(propsQa)}` });
      if (!propsQa.ok) {
        // FIXME(logger): migrate to logPipeline*
        console.warn(
