...
export async function runFullChapterPipelineFromJob(jobId: string) {
  ...
  logPipelineInfo("pipeline.config.default_mymanga", { SUPABASE_URL: !!process.env.NEXT_PUBLIC_SUPABASE_URL, SERVICE_ROLE: !!process.env.SUPABASE_SERVICE_ROLE_KEY, ANON_KEY: !!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY, message: `SUPABASE_URL=${!!process.env.NEXT_PUBLIC_SUPABASE_URL} SERVICE_ROLE=${!!process.env.SUPABASE_SERVICE_ROLE_KEY} ANON_KEY=${!!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY} BUCKET=${process.env.STORAGE_BUCKET ?? "(default:MyManga)"}` });
  ...
  logPipelineInfo("pipeline.T12", { pipelineVersion: pipelineVersion, project: projectId, job: jobId });
    ...
    try {
      ...
      if (persistedIntentContract) {
        try {
          ...
          logPipelineWarn("full.chapter.pipeline.persisted_intent_narrative_contract_invalid_falling_back", { value: parseErr,
           });
        }
      }
  ...
  try {
    const { v3RenderSucceeded, visualWorldDiscovery, pipelineUserWarnings } = await runPremiumV3Pipeline({
      ...
      rawCharacters: rawCharacters.map((c) => {
        ...
        return {
          ...
          visualRefs: c.visualRefsPayload,
          visualLocks: c.visualLocksPayload,
          canonPack: c.canonPackPayload,
          loraAttachments: c.loraAttachmentsPayload,
        };
      }),
    ...
    // Le pipeline legacy a été supprimé : v3 est désormais le SEUL chemin.
    // Si le rendu v3 n'a pas pleinement réussi, on échoue dur — il n'existe
    // plus de retombée narrative/image legacy à câbler.
    if (!v3RenderSucceeded) {
      throw new Error(
        "premium_v3_render_failed: le pipeline v3 n'a pas rendu intégralement et le fallback legacy a été retiré. " +
          "Échec dur — corriger la cause racine v3.",
      );
    }
    ...
    const autoLoraQueued = await queueAutoLoraTrainingIfEligible({
      ...
      logPipelineError("auto.lora.queue_failed", { project: projectId, error: msg });
      return 0;
    });
    if (autoLoraQueued > 0) {
      logPipelineInfo("auto.lora", { queued_for_project: projectId, count: autoLoraQueued });
    }

    // FIXME(logger): migrate to logPipeline*
    console.log(
      `[pipeline:v3] premium_v3_finalized chapterId=${chapterId} — pipeline v3 unique, aucun legacy.`,
    );
