...
export async function queueAutoLoraTrainingIfEligible(input: {
  ...
  for (const candidate of candidates) {
    ...
    logPipelineInfo("auto.lora.queued", { character: candidate.name, refs: seedRefs.length });
    ...
    void (async () => {
      ...
      if (!result.ok) {
        ...
        logPipelineError("auto.lora.failed__unknown", { character: candidate.name, message: `failed character=${candidate.name} error=${result.error ?? "unknown"}` });
        return;
      }
      ...
      logPipelineInfo("auto.lora.ready", { character: candidate.name });
    })().catch((error) => {
      const message = error instanceof Error ? error.message : "auto_lora_background_error";
      logPipelineError("auto.lora.background_crash", { character: candidate.name, error: message });
    });
  }
