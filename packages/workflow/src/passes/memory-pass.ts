...
export async function runMemoryPass(
  ...
  logPipelineInfo("pipeline.continuity_score", { message: `Continuity score: ${continuityReport.score.toFixed(2)}` });
  ...
  logPipelineInfo("pipeline.canon_state_persisted_with_warnings", { message: `Canon state persisted with ${continuityReport.issues.length} warnings` });
