...
export function resolveEffectiveLocationCanon(input: {
  ...
  return {
    canon: {
      ...
      mustKeep: studio?.mustKeep?.length ? studio.mustKeep : fallback.mustKeep ?? [],
      forbiddenDrift: studio?.forbiddenDrift?.length ? studio.forbiddenDrift : fallback.forbiddenDrift ?? [],
      isPrimary: studio?.isPrimary ?? fallback.isPrimary ?? false,
      visualBrief: studio?.visualBrief ?? fallback.visualBrief ?? null,
      locationType: studio?.locationType ?? fallback.locationType ?? null,
      fixedElements: studio?.fixedElements?.length ? studio.fixedElements : fallback.fixedElements ?? [],
      lightingRules: studio?.lightingRules?.length ? studio.lightingRules : fallback.lightingRules ?? [],
      palette: studio?.palette?.length ? studio.palette : fallback.palette ?? [],
      referenceImages: studio?.referenceImages?.length ? studio.referenceImages : fallback.referenceImages ?? [],
