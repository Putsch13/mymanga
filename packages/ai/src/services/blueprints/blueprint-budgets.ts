...
/** Contexte cast studio pour pénalités readiness (héros 2 / déuteragoniste). */
...
function panelBlueprintReferencesCharacterId(bp: PanelBlueprintPremium, characterId: string): boolean {
  ...
  if (Array.isArray(bp.characterVisualDna) && bp.characterVisualDna.some((d) => d.characterId === cid)) {
    return true;
  }
  return false;
}

function countPanelsReferencingCharacter(
  blueprints: PanelBlueprintPremium[],
  characterId: string | null | undefined,
): number {
  if (!characterId?.trim()) return 0;
  return blueprints.filter((bp) => panelBlueprintReferencesCharacterId(bp, characterId)).length;
}
