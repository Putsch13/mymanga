...
describe("buildMinimalPanelPrompt", () => {
  ...
  it("n'injecte PAS les trigger words LoRA dans le bloc STYLE (dédup — injection unique en tête côté générateur)", () => {
    const r = buildMinimalPanelPrompt(
      makeSpec({
        loraBindings: [
          {
    ...
    // Le trigger word est ajouté UNE SEULE fois, en préfixe, par
    // injectLoraTriggerWords (default-panel-image-generator). Le bloc STYLE
    // ne doit plus le dupliquer.
    expect(r.positive).not.toContain("LoRA cues:");
    expect(r.positive.toLowerCase()).not.toContain("miyastyle");
  });
