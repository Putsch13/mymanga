...
export type ComposeVisualWorldContractInput = {
  ...
  /** Époque + cadre (ChapterIntentContract) — décors/props/costumes cohérents. */
  era?: string | null;
  setting?: string | null;
...
function buildSystemPrompt(): string {
  return [
    ...
    "ANCRE TEMPORELLE STRICTE : si les champs `era` et/ou `setting` sont fournis, TOUS les lieux, architectures, props, véhicules et costumes implicites DOIVENT respecter cette époque et ce cadre. Aucun anachronisme (pas de néon ni d'arme à feu en médiéval, pas de torche ni d'épée dans une station orbitale). En cas d'absence, déduis l'époque la plus cohérente avec l'histoire.",
...
function buildUserPayload(input: ComposeVisualWorldContractInput): string {
  ...
  return JSON.stringify({
    ...
    // ANCRE FORTE — tous les décors, props et architectures DOIVENT respecter
    // cette époque/cadre (pas d'anachronisme : pas de néon en médiéval, pas de
    // torche dans une station orbitale).
    era: input.era ?? "",
