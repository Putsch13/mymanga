...
export function buildCutawayPlan(blueprints: ReadonlyArray<PanelBlueprintPremium>): CutawayPlan {
...
export function runCutawayQa(
  ...
  const dangerBeats = [...beatIds].filter((id) => {
    ...
    const hasDangerCutaway = cutawayPlan.panels.some(
      (p) => p.beatId === id && (p.type === "danger_signal" || p.type === "environment_escalation"),
    );
