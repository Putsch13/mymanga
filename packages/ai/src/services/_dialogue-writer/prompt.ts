...
/**
 * Sérialise un `speechProfile` (Record libre) en clauses lisibles
 * "clé: valeur; clé: valeur", au lieu d'un blob JSON tronqué à 350 chars
 * (qui coupait en plein objet → fragment malformé `{"tone":"sarcas`).
 * Coupe propre à une frontière de clause, jamais au milieu d'un mot/JSON.
 */
export function serializeSpeechProfileForPrompt(
  profile: Record<string, unknown> | null | undefined,
  maxChars = 320,
): string {
  if (!profile || typeof profile !== "object") return "voix non précisée";
  ...
  for (const [key, value] of Object.entries(profile)) {
    ...
    if (typeof value === "string") {
      ...
      if (items.length > 0) rendered = items.slice(0, 4).join(", ");
    }
    // On ignore volontairement les objets imbriqués (bruit pour le LLM).
    if (rendered) clauses.push(`${key}: ${rendered}`);
  }
  if (clauses.length === 0) return "voix non précisée";
  let out = "";
  for (const clause of clauses) {
    const next = out ? `${out}; ${clause}` : clause;
    if (next.length > maxChars) break; // coupe à la frontière de clause
    out = next;
  }
  return out || clauses[0]!.slice(0, maxChars);
}

export function buildDialoguePrompt(input: DialogueWriterInput): string {
  const characterList = input.characters
    .map((c) => {
      const voice = serializeSpeechProfileForPrompt(c.speechProfile);
