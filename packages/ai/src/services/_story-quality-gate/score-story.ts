/**
 * Sous-scores dérivés du `GeneratedChapterBundle` + `ChapterDramaticSpine`.
 * Chacun renvoie `{ score: 0..100, issues: StoryQualityIssue[] }`.
 */
import type { GeneratedChapterBundle } from "../../chapter/shared-types";
import type { ChapterDramaticSpine } from "../story-spine";
import type { StoryQualityIssue } from "./types";

export function scoreCausality(
  bundle: GeneratedChapterBundle,
): { score: number; issues: StoryQualityIssue[] } {
...
export function scoreBeatVariety(
  ...
  if (totalBeats === 0) {
    return {
      score: 0,
      issues: [
        { code: "no_beats", severity: "error", message: "Aucun beat narratif identifié." },
      ],
    };
  }
...
export function scoreMicroTurns(
  bundle: GeneratedChapterBundle,
): { score: number; issues: StoryQualityIssue[] } {
  const issues: StoryQualityIssue[] = [];
  const scenes = bundle.script?.scenes ?? [];

  const microTurnWords = [
    "mais",
    "cependant",
    "pourtant",
    "soudain",
...
export function scoreCliffhanger(
  spine: ChapterDramaticSpine,
  bundle: GeneratedChapterBundle,
): { score: number; issues: StoryQualityIssue[] } {
  const issues: StoryQualityIssue[] = [];
  ...
  if (spine.cliffhangerPrep) {
    ...
    issues.push({
      ...
      severity: "warning",
      message:
        "Aucun cliffhanger préparé détecté. Ajouter un beat cliff_pivot ou reveal en fin de chapitre.",
    });
  }
...
export function scorePayoff(
  spine: ChapterDramaticSpine,
): { score: number; issues: StoryQualityIssue[] } {
  const issues: StoryQualityIssue[] = [
    ...spine.payoffWeaknesses.map((w) => ({
  ...
  const hasPayoffBeats = spine.beats.some((b) =>
    ["reveal", "cliff_pivot", "silent_aftermath"].includes(b.eventType),
  );
...
export function scoreBreathing(
  spine: ChapterDramaticSpine,
): { score: number; issues: StoryQualityIssue[] } {
  const issues: StoryQualityIssue[] = [];
  const totalBeats = spine.beats.length;
  ...
  const actionBeats = spine.beats.filter((b) =>
    ["confrontation", "escalation", "counterattack", "fast_exchange"].includes(b.eventType),
  ).length;
  const restBeats = spine.beats.filter((b) =>
    ["silent_aftermath", "setup", "near_confession"].includes(b.eventType),
  ).length;
...
export function scoreSceneUtility(
  bundle: GeneratedChapterBundle,
): { score: number; issues: StoryQualityIssue[] } {
  const issues: StoryQualityIssue[] = [];
  const scenes = bundle.script?.scenes ?? [];
...
export function scoreCharacterFunction(
  bundle: GeneratedChapterBundle,
): { score: number; issues: StoryQualityIssue[] } {
  const issues: StoryQualityIssue[] = [];
  const scenes = bundle.script?.scenes ?? [];
