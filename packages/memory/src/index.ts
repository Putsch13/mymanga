...
export async function buildProjectContext(
  ...
  const projectLocations = await prisma.location.findMany({
    where: { projectId },
    orderBy: { name: "asc" },
    take: 50,
    select: {
...
// Ces helpers permettent de retrouver sémantiquement un personnage, un décor
// ou un PNJ persistant lorsqu'un chapitre ultérieur réfère vaguement à
// « la forge du village » ou « le garde borgne » : l'embedding pointe vers
// la fiche canonique au lieu de laisser le LLM inventer une nouvelle entrée.
...
export async function indexCharacterForRag(
  ...
  return replaceRagDocument(prisma, {
    ...
    content: parts.join("\n"),
    metadata: { characterId: character.id, roleType: character.roleType ?? null },
  });
}
...
export async function indexNpcForRag(
  prisma: PrismaClient,
  npc: {
    id: string;
    projectId: string;
  ...
  return replaceRagDocument(prisma, {
    ...
    content: parts.join("\n"),
    metadata: { npcId: npc.id, role: npc.role ?? null, locationKey: npc.locationKey ?? null },
  });
}
